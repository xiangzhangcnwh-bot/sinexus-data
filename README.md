...
```bash
git clone https://github.com/YOUR_USER/sinexus-data
cd sinexus-data
...
```bash
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/sinexus-data/main/install.sh | bash
```

完整版（含 MinerU 解析 + BGE 嵌入）：

```bash
curl -fsSL https://raw.githubusercontent.com/YOUR_USER/sinexus-data/main/install.sh | bash -s -- --full
```
